...
> 백엔드와 공유하는 정확한 JSON 구조·고정 헤더·변경 규칙은 [공통 API 계약](docs/api_contract.md)이 단일 원본입니다. 이 README나 다른 설명이 다르면 공통 계약을 기준으로 양쪽 구현과 테스트를 함께 수정합니다.

라이브커머스 구매자가 사이트 안에서 YouTube 라이브 영상과 공식 동기화 채팅을 보면서
Redis 대기실을 거쳐 서버 견적을 확인하고 계좌이체 주문을 만든 뒤, 비밀 조회키로
입금·배송 상태를 확인하는 Next.js 구매 프런트엔드입니다. 상품 관리자는 화면에 직접
입력한 API 키로 카탈로그 상품, 옵션 SKU, 판매자, 브랜드·제조사, 이미지·속성 및
창고별 재고/입고예정을 조회·저장할 수 있습니다.
구매자는 쿠폰을 적용하고 상품의 3단계 분류·입고예정일을 확인할 수 있으며, 백엔드는
결제완료 주문을 가명 통계로 집계합니다.
...
Vercel Preview 전체를 와일드카드로 허용하지 마세요. Preview는 별도 테스트 API와 명시적으로 승인한 Preview Origin을 사용합니다. FastAPI가 `Content-Type`, `X-Waiting-Room-Ticket`, `X-Waiting-Room-Token`, `X-Order-Token`, `X-Purchase-Offer-Token`, `X-Admin-API-Key` 요청 헤더를 허용하고 `Retry-After` 응답 헤더를 CORS에 노출해야 합니다.

백엔드의 `BROADCAST_EMBED_ORIGIN`은 이 프런트의 실제 Origin과 같아야 공식 YouTube
라이브 채팅 iframe이 열립니다. `BROADCAST_VIDEO_ID`를 비우면 `YOUTUBE_VIDEO_ID`를
상속하므로 플레이어와 채팅이 같은 방송에 동기화됩니다. YouTube 정책상 모바일 웹에서는
공식 채팅 iframe을 지원하지 않는 모바일에서는 자체 채팅에 작성하고 공식 YouTube API
메시지를 읽기 전용으로 함께 표시합니다. 자체 통합 채팅이 꺼진 경우에만 `watch_url` 기반
“YouTube에서 채팅 참여” 버튼을 fallback으로 표시합니다.
...
- 쿠폰 코드는 견적 요청에 포함하며 할인 전 금액·할인액·최종 금액은 서버 응답을 그대로 사용합니다. 상품 카드에는 서버 카탈로그의 분류와 입고예정일을 표시합니다.
...
- 채팅 세션 토큰은 24시간 만료시각과 함께 `sessionStorage`에만 저장합니다. 자체 메시지는 신고할 수 있고 YouTube 메시지는 읽기 전용이며 두 출처를 항상 표시합니다.
- 관리자 통계 UI를 추가할 때는 `/admin/api/analytics`의 가명·최소집단 적용 응답만 사용하고 주문 개인정보나 운영 조회지문을 브라우저에 결합하지 않습니다.
...
6. FastAPI의 `BROADCAST_EMBED_ORIGIN`을 Production 프런트 Origin으로 설정합니다.
7. CSP를 처음 변경할 때는 report-only 환경에서 위반을 확인한 후 enforcement를 배포합니다.
