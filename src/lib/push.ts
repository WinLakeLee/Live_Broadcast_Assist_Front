...
export async function requestPushToken(
  ...
  return new Promise((resolve) => {
    const receive = (event: Event) => {
      ...
      cleanup();
      resolve(typeof token === "string" && token.trim() ? token : null);
    };
    ...
    const cleanup = () => {
      clearTimeout(timer);
      window.removeEventListener("live-purchase:push-token", receive);
    };
    window.addEventListener("live-purchase:push-token", receive, {
      once: true,
    });
