  ...
  try {
    if (apiBaseUrl) {
      ...
      if (process.env.NODE_ENV === "production" && parsed.protocol !== "https:")
        error = "운영 환경의 주문 API 주소는 HTTPS여야 합니다.";
    }
  } catch {
    error = "주문 API 주소 형식이 올바르지 않습니다.";
  }
  if (process.env.NODE_ENV === "production" && (!apiBaseUrl || !key))
    error =
      "운영 환경의 주문 API 주소 또는 Turnstile 사이트 키가 설정되지 않았습니다.";
  return {
    apiBaseUrl: apiBaseUrl || "http://localhost:8000",
    turnstileSiteKey: key,
    brandName: process.env.NEXT_PUBLIC_BRAND_NAME || "라이브 구매",
