...
import { depositorResultSchema, orderCreatedSchema, orderStatusSchema, quoteSchema, type OrderItem, type StockPolicy, type TicketCredentials } from "./contracts";
...
export const registerDepositor = (reference: string, token: string, input: { depositor_name: string; bank_name: string }, signal?: AbortSignal) =>
  apiRequest(`/orders/${encodeURIComponent(reference)}/depositor`, depositorResultSchema, { method: "POST", headers: { "X-Order-Token": token }, body: JSON.stringify(input), signal });
