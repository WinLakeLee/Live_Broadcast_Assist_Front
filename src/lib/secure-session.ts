import type { OrderCreatedData, QuoteData, StockPolicy, WaitingTicket } from "./api/contracts";
...
export type PurchaseDraft = { items: Array<{ product_name: string; quantity: number }>; buyerName: string; phone: string; address: string; stockPolicy: StockPolicy };
...
export function saveReview(review: StoredReview) { storage()?.setItem(REVIEW_KEY, JSON.stringify(review)); }
export function loadReview(): StoredReview | null { try { const raw = storage()?.getItem(REVIEW_KEY); return raw ? JSON.parse(raw) as StoredReview : null; } catch { return null; } }
export function clearReview() { storage()?.removeItem(REVIEW_KEY); }
export function savePurchaseDraft(draft: PurchaseDraft) { storage()?.setItem(DRAFT_KEY, JSON.stringify(draft)); }
export function loadPurchaseDraft(): PurchaseDraft | null { try { const raw = storage()?.getItem(DRAFT_KEY); return raw ? JSON.parse(raw) as PurchaseDraft : null; } catch { return null; } }
export function clearPurchaseDraft() { storage()?.removeItem(DRAFT_KEY); }
