...
export function PaymentStatus({data}:{data:OrderStatusData|PaymentMatchData}){const item=copy[data.status]??{title:data.status,detail:"서버에서 확인한 현재 상태입니다.",kind:"warn" as const};const full="items" in data;const trackingUrl=safeTrackingUrl(data.courier?.tracking_url);const hasTracking=full&&data.items.some(line=>Boolean(line.tracking_number));return <section className="card" aria-live="polite"><span className={`status-badge ${item.kind==="ok"?"ok":""}`}>{item.kind==="ok"?<CheckCircle2 size={15}/>:item.kind==="danger"?<AlertTriangle size={15}/>:<Clock3 size={15}/>} {data.status}</span><h2>{item.title}</h2><p>{item.detail}</p><div className="summary-row"><span>입금 예정액</span><strong>{formatMoney(data.expected_amount)}</strong></div><div className="summary-row"><span>확인된 입금액</span><strong>{formatMoney(data.paid_amount)}</strong></div><div className="summary-row"><span>차액</span><strong>{describeDifference(data.difference)}</strong></div>{hasTracking&&data.courier&&<div className="notice success"><strong>{data.courier.display_name}</strong><br/>{trackingUrl&&<a href={trackingUrl} target="_blank" rel="noreferrer">공식 배송조회 페이지 열기</a>}</div>}{full&&<><p className="muted">주문일 {formatDateTime(data.created_at)}</p><h3><PackageCheck size={19}/> 상품 상태</h3>{data.items.map((line,i)=><div className="quote-line" key={`${line.product_name}-${i}`}><span><strong>{line.product_name}</strong> × {line.quantity}<br/><small>{line.status}{line.cancellation_reason&&` · ${line.cancellation_reason}`}{line.tracking_number&&` · ${data.courier?.display_name??"택배"} 운송장 ${line.tracking_number}`}</small></span><strong>{formatMoney(line.price)}</strong></div>)}</>}</section>}
